...
class Xb6vPlugin(BasePlugin):
    name = "xb6v"
    display_name = "6V电影网"
    version = "1.0.0"
    author = "pan-relay"
    description = "6V电影网 数据源搜索插件"
    priority = 140
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://www.66ss.org/search/%s.html" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://www.66ss.org"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://www.66ss.org", headers={"User-Agent": user_agents[0]}, timeout=3.0)
