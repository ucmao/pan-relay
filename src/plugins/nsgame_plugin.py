...
class NsgamePlugin(BasePlugin):
    name = "nsgame"
    display_name = "NSGame游戏"
    version = "1.0.0"
    author = "pan-relay"
    description = "NSGame游戏 数据源搜索插件"
    priority = 130
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://nsgame.com/search?q=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://nsgame.com"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://nsgame.com", headers={"User-Agent": user_agents[0]}, timeout=3.0)
