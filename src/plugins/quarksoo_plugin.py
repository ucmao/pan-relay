...
class QuarksooPlugin(BasePlugin):
    name = "quarksoo"
    display_name = "夸克搜"
    version = "1.0.0"
    author = "pan-relay"
    description = "夸克搜 数据源搜索插件"
    priority = 140
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://pan.quarksoo.cn/api/search?q=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://pan.quarksoo.cn"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://pan.quarksoo.cn", headers={"User-Agent": user_agents[0]}, timeout=3.0)
