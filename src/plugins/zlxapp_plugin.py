...
class ZlxappPlugin(BasePlugin):
    name = "zlxapp"
    display_name = "智辽小App"
    version = "1.0.0"
    author = "pan-relay"
    description = "智辽小App 数据源搜索插件"
    priority = 130
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://zlxapp.com/api/search?q=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://zlxapp.com"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://zlxapp.com", headers={"User-Agent": user_agents[0]}, timeout=3.0)
