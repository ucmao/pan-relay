...
class WujiPlugin(BasePlugin):
    name = "wuji"
    display_name = "无极磁链"
    version = "1.0.0"
    author = "pan-relay"
    description = "无极磁链 数据源搜索插件"
    priority = 130
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://xcili.net/search/%s.html" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://xcili.net"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://xcili.net", headers={"User-Agent": user_agents[0]}, timeout=3.0)
