...
class XysPlugin(BasePlugin):
    name = "xys"
    display_name = "新云搜"
    version = "1.0.0"
    author = "pan-relay"
    description = "新云搜 数据源搜索插件"
    priority = 130
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://www.yunso.net/api/search?q=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://www.yunso.net"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://www.yunso.net", headers={"User-Agent": user_agents[0]}, timeout=3.0)
