...
class AhhhhfsPlugin(BasePlugin):
    name = "ahhhhfs"
    display_name = "A4k影视资讯"
    version = "1.0.0"
    author = "pan-relay"
    description = "A4k影视资讯 数据源搜索插件"
    priority = 140
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://www.ahhhhfs.com/?s=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://www.ahhhhfs.com"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://www.ahhhhfs.com", headers={"User-Agent": user_agents[0]}, timeout=3.0)
