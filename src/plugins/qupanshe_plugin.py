...
class QupanshePlugin(BasePlugin):
    name = "qupanshe"
    display_name = "趣盘社"
    version = "1.0.0"
    author = "pan-relay"
    description = "趣盘社 数据源搜索插件"
    priority = 140
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://www.qupanshe.com/search?keyword=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://www.qupanshe.com"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://www.qupanshe.com", headers={"User-Agent": user_agents[0]}, timeout=3.0)
