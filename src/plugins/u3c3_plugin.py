...
class U3c3Plugin(BasePlugin):
    name = "u3c3"
    display_name = "U3c3资源"
    version = "1.0.0"
    author = "pan-relay"
    description = "U3c3资源 数据源搜索插件"
    priority = 120
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://u3c3.com/search?q=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://u3c3.com"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://u3c3.com", headers={"User-Agent": user_agents[0]}, timeout=3.0)
