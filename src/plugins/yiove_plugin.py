...
class YiovePlugin(BasePlugin):
    name = "yiove"
    display_name = "Yiove社区"
    version = "1.0.0"
    author = "pan-relay"
    description = "Yiove社区 数据源搜索插件"
    priority = 120
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://bbs.yiove.com/search?q=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://bbs.yiove.com"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://bbs.yiove.com", headers={"User-Agent": user_agents[0]}, timeout=3.0)
