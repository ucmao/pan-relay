...
class JavdbPlugin(BasePlugin):
    name = "javdb"
    display_name = "JavDB资源"
    version = "1.0.0"
    author = "pan-relay"
    description = "JavDB资源 数据源搜索插件"
    priority = 120
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://javdb.com/search?q=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://javdb.com"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://javdb.com", headers={"User-Agent": user_agents[0]}, timeout=3.0)
