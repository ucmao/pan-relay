...
class HdmoliPlugin(BasePlugin):
    name = "hdmoli"
    display_name = "HD摩利影视"
    version = "1.0.0"
    author = "pan-relay"
    description = "HD摩利影视 数据源搜索插件"
    priority = 150
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://www.hdmoli.com/search/%s.html" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://www.hdmoli.com"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://www.hdmoli.com", headers={"User-Agent": user_agents[0]}, timeout=3.0)
