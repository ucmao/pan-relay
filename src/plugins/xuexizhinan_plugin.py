...
class XuexizhinanPlugin(BasePlugin):
    name = "xuexizhinan"
    display_name = "学习指南网"
    version = "1.0.0"
    author = "pan-relay"
    description = "学习指南网 数据源搜索插件"
    priority = 130
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://xuexizhinan.com/?post_type=book&s=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://xuexizhinan.com"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://xuexizhinan.com", headers={"User-Agent": user_agents[0]}, timeout=3.0)
