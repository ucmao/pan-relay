...
class DjgouPlugin(BasePlugin):
    name = "djgou"
    display_name = "短剧狗"
    version = "1.0.0"
    author = "pan-relay"
    description = "短剧狗 数据源搜索插件"
    priority = 140
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://duanjugou.top/search?wd=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://duanjugou.top"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://duanjugou.top", headers={"User-Agent": user_agents[0]}, timeout=3.0)
