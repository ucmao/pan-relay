...
class Lou1Plugin(BasePlugin):
    name = "lou1"
    display_name = "楼一影音"
    version = "1.0.0"
    author = "pan-relay"
    description = "楼一影音 数据源搜索插件"
    priority = 130
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "http://www.1lou.me/search?q=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "http://www.1lou.me"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("http://www.1lou.me", headers={"User-Agent": user_agents[0]}, timeout=3.0)
