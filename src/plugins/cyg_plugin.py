...
class CygPlugin(BasePlugin):
    name = "cyg"
    display_name = "CYG社区"
    version = "1.0.0"
    author = "pan-relay"
    description = "CYG社区 数据源搜索插件"
    priority = 130
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://cyg.app/wp-json/wp/v2/posts?search=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://cyg.app"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://cyg.app", headers={"User-Agent": user_agents[0]}, timeout=3.0)
