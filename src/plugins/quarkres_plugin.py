...
class QuarkresPlugin(BasePlugin):
    name = "quarkres"
    display_name = "夸克资源站"
    version = "1.0.0"
    author = "pan-relay"
    description = "夸克资源站 数据源搜索插件"
    priority = 140
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://quarkres.com/api/search?q=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://quarkres.com"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://quarkres.com", headers={"User-Agent": user_agents[0]}, timeout=3.0)
