...
class QuarktvPlugin(BasePlugin):
    name = "quarktv"
    display_name = "夸克TV"
    version = "1.0.0"
    author = "pan-relay"
    description = "夸克TV 数据源搜索插件"
    priority = 140
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://www.quarktv.com/search?wd=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://www.quarktv.com"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://www.quarktv.com", headers={"User-Agent": user_agents[0]}, timeout=3.0)
