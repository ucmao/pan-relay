...
class KkvPlugin(BasePlugin):
    name = "kkv"
    display_name = "KKV影视网"
    version = "1.0.0"
    author = "pan-relay"
    description = "KKV影视网 数据源搜索插件"
    priority = 140
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "http://kkv.q-23.cn/search?kw=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "http://kkv.q-23.cn"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("http://kkv.q-23.cn", headers={"User-Agent": user_agents[0]}, timeout=3.0)
