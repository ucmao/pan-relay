...
class MikuclubPlugin(BasePlugin):
    name = "mikuclub"
    display_name = "MikuClub"
    version = "1.0.0"
    author = "pan-relay"
    description = "MikuClub 数据源搜索插件"
    priority = 130
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://mikuclub.org/search?q=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://mikuclub.org"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://mikuclub.org", headers={"User-Agent": user_agents[0]}, timeout=3.0)
