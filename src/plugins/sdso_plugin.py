...
class SdsoPlugin(BasePlugin):
    name = "sdso"
    display_name = "闪电搜资源"
    version = "1.0.0"
    author = "pan-relay"
    description = "闪电搜资源 数据源搜索插件"
    priority = 130
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://sdso.top/api/sd/search?name=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://sdso.top"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://sdso.top", headers={"User-Agent": user_agents[0]}, timeout=3.0)
