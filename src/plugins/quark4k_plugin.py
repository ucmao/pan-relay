...
class Quark4kPlugin(BasePlugin):
    name = "quark4k"
    display_name = "夸克4K影视"
    version = "1.0.0"
    author = "pan-relay"
    description = "夸克4K影视 数据源搜索插件"
    priority = 150
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://quark4k.com/api/discussions?filter[q]=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://quark4k.com"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://quark4k.com", headers={"User-Agent": user_agents[0]}, timeout=3.0)
