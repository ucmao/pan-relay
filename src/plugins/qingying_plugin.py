...
class QingyingPlugin(BasePlugin):
    name = "qingying"
    display_name = "晴影视"
    version = "1.0.0"
    author = "pan-relay"
    description = "晴影视 数据源搜索插件"
    priority = 130
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "http://revohd.com/search/%s.html" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "http://revohd.com"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("http://revohd.com", headers={"User-Agent": user_agents[0]}, timeout=3.0)
