...
class PansearchPlugin(BasePlugin):
    name = "pansearch"
    display_name = "PanSearch搜索"
    version = "1.0.0"
    author = "pan-relay"
    description = "PanSearch搜索 数据源搜索插件"
    priority = 160
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://www.pansearch.me/search?keyword=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://www.pansearch.me"}
        try:
            ...
            if "json" in ct or resp.text.strip().startswith(("{", "[")):
                try:
                    ...
                    for item in items:
                        ...
                        link = item.get("url") or item.get("link") or item.get("share_link") or item.get("pan_url") or ""
                        if title and link:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://www.pansearch.me", headers={"User-Agent": user_agents[0]}, timeout=3.0)
