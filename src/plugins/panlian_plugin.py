...
class PanlianPlugin(BasePlugin):
    name = "panlian"
    display_name = "盘链搜索"
    version = "1.0.0"
    author = "pan-relay"
    description = "盘链搜索 数据源搜索插件"
    priority = 140
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://panlian.net/search?q=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://panlian.net"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://panlian.net", headers={"User-Agent": user_agents[0]}, timeout=3.0)
