...
class MelostPlugin(BasePlugin):
    name = "melost"
    display_name = "Melost音乐影音"
    version = "1.0.0"
    author = "pan-relay"
    description = "Melost音乐影音 数据源搜索插件"
    priority = 150
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://www.melost.cn/v1/search/disk?kw=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://www.melost.cn"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://www.melost.cn", headers={"User-Agent": user_agents[0]}, timeout=3.0)
