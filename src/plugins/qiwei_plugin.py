...
class QiweiPlugin(BasePlugin):
    name = "qiwei"
    display_name = "七味影视"
    version = "1.0.0"
    author = "pan-relay"
    description = "七味影视 数据源搜索插件"
    priority = 130
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://www.qnmp4.com/search?wd=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://www.qnmp4.com"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://www.qnmp4.com", headers={"User-Agent": user_agents[0]}, timeout=3.0)
