...
class DiduanPlugin(BasePlugin):
    name = "diduan"
    display_name = "低端影视"
    version = "1.0.0"
    author = "pan-relay"
    description = "低端影视 数据源搜索插件"
    priority = 140
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://ddys.pro/?s=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://ddys.pro"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://ddys.pro", headers={"User-Agent": user_agents[0]}, timeout=3.0)
