...
class HubanPlugin(BasePlugin):
    name = "huban"
    display_name = "虎板资源"
    version = "1.0.0"
    author = "pan-relay"
    description = "虎板资源 数据源搜索插件"
    priority = 130
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://huban.org/search?q=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://huban.org"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://huban.org", headers={"User-Agent": user_agents[0]}, timeout=3.0)
