...
class Ting77Plugin(BasePlugin):
    name = "ting77"
    display_name = "77听书"
    version = "1.0.0"
    author = "pan-relay"
    description = "77听书 数据源搜索插件"
    priority = 130
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://sou.77ting.top/api/search?q=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://sou.77ting.top"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://sou.77ting.top", headers={"User-Agent": user_agents[0]}, timeout=3.0)
