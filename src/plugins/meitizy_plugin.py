...
class MeitizyPlugin(BasePlugin):
    name = "meitizy"
    display_name = "美剧资源网"
    version = "1.0.0"
    author = "pan-relay"
    description = "美剧资源网 数据源搜索插件"
    priority = 140
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://apis.451024.xyz/search?keyword=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://apis.451024.xyz"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://apis.451024.xyz", headers={"User-Agent": user_agents[0]}, timeout=3.0)
