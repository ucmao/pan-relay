...
class DaishudjPlugin(BasePlugin):
    name = "daishudj"
    display_name = "袋鼠短剧"
    version = "1.0.0"
    author = "pan-relay"
    description = "袋鼠短剧 数据源搜索插件"
    priority = 140
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://www.daishuduanju.com/?s=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://www.daishuduanju.com"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://www.daishuduanju.com", headers={"User-Agent": user_agents[0]}, timeout=3.0)
