...
class SusuPlugin(BasePlugin):
    name = "susu"
    display_name = "苏苏分享"
    version = "1.0.0"
    author = "pan-relay"
    description = "苏苏分享 数据源搜索插件"
    priority = 130
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://susuifa.com/?type=post&s=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://susuifa.com"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://susuifa.com", headers={"User-Agent": user_agents[0]}, timeout=3.0)
