...
class Pan666Plugin(BasePlugin):
    name = "pan666"
    display_name = "666盘搜"
    version = "1.0.0"
    author = "pan-relay"
    description = "666盘搜 数据源搜索插件"
    priority = 140
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://pan666.net/search?q=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://pan666.net"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://pan666.net", headers={"User-Agent": user_agents[0]}, timeout=3.0)
