...
class PanyqPlugin(BasePlugin):
    name = "panyq"
    display_name = "盘圈搜索"
    version = "1.0.0"
    author = "pan-relay"
    description = "盘圈搜索 数据源搜索插件"
    priority = 130
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://panyq.com/search?q=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://panyq.com"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://panyq.com", headers={"User-Agent": user_agents[0]}, timeout=3.0)
