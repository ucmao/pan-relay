...
class QqpdPlugin(BasePlugin):
    name = "qqpd"
    display_name = "QQ频道资源"
    version = "1.0.0"
    author = "pan-relay"
    description = "QQ频道资源 数据源搜索插件"
    priority = 130
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://pd.qq.com/search?keyword=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://pd.qq.com"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://pd.qq.com", headers={"User-Agent": user_agents[0]}, timeout=3.0)
