...
class LingjispPlugin(BasePlugin):
    name = "lingjisp"
    display_name = "零极视频"
    version = "1.0.0"
    author = "pan-relay"
    description = "零极视频 数据源搜索插件"
    priority = 140
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://web5.mukaku.com/prod/api/v1/search?kw=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://web5.mukaku.com"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://web5.mukaku.com", headers={"User-Agent": user_agents[0]}, timeout=3.0)
