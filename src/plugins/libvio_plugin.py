...
class LibvioPlugin(BasePlugin):
    name = "libvio"
    display_name = "LIBVIO追剧网"
    version = "1.0.0"
    author = "pan-relay"
    description = "LIBVIO追剧网 数据源搜索插件"
    priority = 150
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://libvio.host/search/-------------.html?wd=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://libvio.host"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://libvio.host", headers={"User-Agent": user_agents[0]}, timeout=3.0)
