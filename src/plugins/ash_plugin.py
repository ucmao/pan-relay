...
class AshPlugin(BasePlugin):
    name = "ash"
    display_name = "ASH影视"
    version = "1.0.0"
    author = "pan-relay"
    description = "ASH影视 数据源搜索插件"
    priority = 130
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://ash.org/search?q=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://ash.org"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://ash.org", headers={"User-Agent": user_agents[0]}, timeout=3.0)
