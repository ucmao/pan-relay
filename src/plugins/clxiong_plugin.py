...
class ClxiongPlugin(BasePlugin):
    name = "clxiong"
    display_name = "磁力熊"
    version = "1.0.0"
    author = "pan-relay"
    description = "磁力熊 数据源搜索插件"
    priority = 130
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://www.cilixiong.org/search?q=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://www.cilixiong.org"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://www.cilixiong.org", headers={"User-Agent": user_agents[0]}, timeout=3.0)
