...
class ThepiratebayPlugin(BasePlugin):
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://thpibay.xyz/search/%s/1/99/0" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://thpibay.xyz"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://thpibay.xyz", headers={"User-Agent": user_agents[0]}, timeout=3.0)
