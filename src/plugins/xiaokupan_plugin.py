...
class XiaokupanPlugin(BasePlugin):
    name = "xiaokupan"
    display_name = "小夸网盘"
    version = "1.0.0"
    author = "pan-relay"
    description = "小夸网盘 数据源搜索插件"
    priority = 140
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://xiaokupan.com/search?q=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://xiaokupan.com"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://xiaokupan.com", headers={"User-Agent": user_agents[0]}, timeout=3.0)
