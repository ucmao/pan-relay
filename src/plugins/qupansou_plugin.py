...
class QupansouPlugin(BasePlugin):
    name = "qupansou"
    display_name = "趣盘搜资源"
    version = "1.0.0"
    author = "pan-relay"
    description = "趣盘搜资源 数据源搜索插件"
    priority = 130
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://v.funletu.com/search?q=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://v.funletu.com"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://v.funletu.com", headers={"User-Agent": user_agents[0]}, timeout=3.0)
