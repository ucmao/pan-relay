...
class YulinshufaPlugin(BasePlugin):
    name = "yulinshufa"
    display_name = "雨林书法"
    version = "1.0.0"
    author = "pan-relay"
    description = "雨林书法 数据源搜索插件"
    priority = 120
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://yulinshufa.com/search?q=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://yulinshufa.com"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://yulinshufa.com", headers={"User-Agent": user_agents[0]}, timeout=3.0)
