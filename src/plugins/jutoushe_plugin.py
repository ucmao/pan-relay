...
class JutoushePlugin(BasePlugin):
    name = "jutoushe"
    display_name = "剧透社资源"
    version = "1.0.0"
    author = "pan-relay"
    description = "剧透社资源 数据源搜索插件"
    priority = 140
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://1.star2.cn/search?wd=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://1.star2.cn"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://1.star2.cn", headers={"User-Agent": user_agents[0]}, timeout=3.0)
