...
class MiosouPlugin(BasePlugin):
    name = "miosou"
    display_name = "Mio搜"
    version = "1.0.0"
    author = "pan-relay"
    description = "Mio搜 数据源搜索插件"
    priority = 150
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://miosou.cc/api/search?keyword=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://miosou.cc"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://miosou.cc", headers={"User-Agent": user_agents[0]}, timeout=3.0)
