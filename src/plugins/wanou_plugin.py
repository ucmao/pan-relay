...
class WanouPlugin(BasePlugin):
    name = "wanou"
    display_name = "玩偶哥哥"
    version = "1.0.0"
    author = "pan-relay"
    description = "玩偶哥哥 数据源搜索插件"
    priority = 150
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://woog.nxog.eu.org/api.php/provide/vod?ac=detail&wd=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://woog.nxog.eu.org"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://woog.nxog.eu.org", headers={"User-Agent": user_agents[0]}, timeout=3.0)
