...
class HaitunsouPlugin(BasePlugin):
    name = "haitunsou"
    display_name = "海豚搜"
    version = "1.0.0"
    author = "pan-relay"
    description = "海豚搜 数据源搜索插件"
    priority = 140
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://haitunsou.com/search?q=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://haitunsou.com"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://haitunsou.com", headers={"User-Agent": user_agents[0]}, timeout=3.0)
