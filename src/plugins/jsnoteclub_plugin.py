...
class JsnoteclubPlugin(BasePlugin):
    name = "jsnoteclub"
    display_name = "JSNoteClub"
    version = "1.0.0"
    author = "pan-relay"
    description = "JSNoteClub 数据源搜索插件"
    priority = 130
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://jsnoteclub.com/search?q=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://jsnoteclub.com"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://jsnoteclub.com", headers={"User-Agent": user_agents[0]}, timeout=3.0)
