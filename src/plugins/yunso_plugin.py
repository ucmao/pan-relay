...
class YunsoPlugin(BasePlugin):
    name = "yunso"
    display_name = "云搜"
    version = "1.0.0"
    author = "pan-relay"
    description = "云搜 数据源搜索插件"
    priority = 140
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://yunso.net/api/search?kw=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://yunso.net"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://yunso.net", headers={"User-Agent": user_agents[0]}, timeout=3.0)
