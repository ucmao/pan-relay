...
class GyingPlugin(BasePlugin):
    name = "gying"
    display_name = "Gying影视"
    version = "1.0.0"
    author = "pan-relay"
    description = "Gying影视 数据源搜索插件"
    priority = 160
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://www.gying.net/search?wd=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://www.gying.net"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://www.gying.net", headers={"User-Agent": user_agents[0]}, timeout=3.0)
