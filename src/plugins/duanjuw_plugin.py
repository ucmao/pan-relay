...
class DuanjuwPlugin(BasePlugin):
    name = "duanjuw"
    display_name = "短剧网"
    version = "1.0.0"
    author = "pan-relay"
    description = "短剧网 数据源搜索插件"
    priority = 140
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://sm3.cc/search?wd=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://sm3.cc"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://sm3.cc", headers={"User-Agent": user_agents[0]}, timeout=3.0)
