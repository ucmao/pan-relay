...
class AlupanPlugin(BasePlugin):
    name = "alupan"
    display_name = "阿里云盘吧"
    version = "1.0.0"
    author = "pan-relay"
    description = "阿里云盘吧 数据源搜索插件"
    priority = 140
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://www.aliupan.com/?s=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://www.aliupan.com"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://www.aliupan.com", headers={"User-Agent": user_agents[0]}, timeout=3.0)
