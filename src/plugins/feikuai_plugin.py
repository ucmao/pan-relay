...
class FeikuaiPlugin(BasePlugin):
    name = "feikuai"
    display_name = "飞快网盘搜索"
    version = "1.0.0"
    author = "pan-relay"
    description = "飞快网盘搜索 数据源搜索插件"
    priority = 140
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://feikuai.tv/t_search/bm_search.php?kw=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://feikuai.tv"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://feikuai.tv", headers={"User-Agent": user_agents[0]}, timeout=3.0)
