...
class YingsoPlugin(BasePlugin):
    name = "yingso"
    display_name = "影搜"
    version = "1.0.0"
    author = "pan-relay"
    description = "影搜 数据源搜索插件"
    priority = 140
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://yingso.org/search?q=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://yingso.org"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://yingso.org", headers={"User-Agent": user_agents[0]}, timeout=3.0)
