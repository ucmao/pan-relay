...
class DiscoursePlugin(BasePlugin):
    name = "discourse"
    display_name = "Discourse论坛"
    version = "1.0.0"
    author = "pan-relay"
    description = "Discourse论坛 数据源搜索插件"
    priority = 130
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://linux.do/search?q=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://linux.do"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://linux.do", headers={"User-Agent": user_agents[0]}, timeout=3.0)
