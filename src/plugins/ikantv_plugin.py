...
class IkantvPlugin(BasePlugin):
    name = "ikantv"
    display_name = "爱看TV资源"
    version = "1.0.0"
    author = "pan-relay"
    description = "爱看TV资源 数据源搜索插件"
    priority = 150
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://api.naspt.vip/api/open/pansou/search?keyword=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://api.naspt.vip"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://api.naspt.vip", headers={"User-Agent": user_agents[0]}, timeout=3.0)
