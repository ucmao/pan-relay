...
class PiankuPlugin(BasePlugin):
    name = "pianku"
    display_name = "片库网"
    version = "1.0.0"
    author = "pan-relay"
    description = "片库网 数据源搜索插件"
    priority = 140
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://btnull.pro/s/%s.html" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://btnull.pro"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://btnull.pro", headers={"User-Agent": user_agents[0]}, timeout=3.0)
