...
class YpfxwPlugin(BasePlugin):
    name = "ypfxw"
    display_name = "云盘分享网"
    version = "1.0.0"
    author = "pan-relay"
    description = "云盘分享网 数据源搜索插件"
    priority = 130
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://ypfxw.com/search.php?q=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://ypfxw.com"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://ypfxw.com", headers={"User-Agent": user_agents[0]}, timeout=3.0)
