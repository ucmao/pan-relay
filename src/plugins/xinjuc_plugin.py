...
class XinjucPlugin(BasePlugin):
    name = "xinjuc"
    display_name = "新剧场"
    version = "1.0.0"
    author = "pan-relay"
    description = "新剧场 数据源搜索插件"
    priority = 130
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://www.xinjuc.com/search?wd=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://www.xinjuc.com"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://www.xinjuc.com", headers={"User-Agent": user_agents[0]}, timeout=3.0)
