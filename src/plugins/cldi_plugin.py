...
class CldiPlugin(BasePlugin):
    name = "cldi"
    display_name = "磁力地带"
    version = "1.0.0"
    author = "pan-relay"
    description = "磁力地带 数据源搜索插件"
    priority = 130
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://wvmzbxki.1122132.xyz/search-%s.html" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://wvmzbxki.1122132.xyz"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://wvmzbxki.1122132.xyz", headers={"User-Agent": user_agents[0]}, timeout=3.0)
