...
class MiaosoPlugin(BasePlugin):
    name = "miaoso"
    display_name = "喵搜"
    version = "1.0.0"
    author = "pan-relay"
    description = "喵搜 数据源搜索插件"
    priority = 140
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://miaosou.fun/api/secendsearch?q=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://miaosou.fun"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://miaosou.fun", headers={"User-Agent": user_agents[0]}, timeout=3.0)
