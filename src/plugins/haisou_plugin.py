...
class HaisouPlugin(BasePlugin):
    name = "haisou"
    display_name = "海搜"
    version = "1.0.0"
    author = "pan-relay"
    description = "海搜 数据源搜索插件"
    priority = 140
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://haisou.cc/api/pan/share/search?query=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://haisou.cc"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://haisou.cc", headers={"User-Agent": user_agents[0]}, timeout=3.0)
