...
class Hdr4kPlugin(BasePlugin):
    name = "hdr4k"
    display_name = "HDR4K影院"
    version = "1.0.0"
    author = "pan-relay"
    description = "HDR4K影院 数据源搜索插件"
    priority = 140
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://hdr4k.com/search?q=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://hdr4k.com"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://hdr4k.com", headers={"User-Agent": user_agents[0]}, timeout=3.0)
