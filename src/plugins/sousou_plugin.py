...
class SousouPlugin(BasePlugin):
    name = "sousou"
    display_name = "搜搜资源"
    version = "1.0.0"
    author = "pan-relay"
    description = "搜搜资源 数据源搜索插件"
    priority = 140
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://sousou.pro/api.php?wd=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://sousou.pro"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://sousou.pro", headers={"User-Agent": user_agents[0]}, timeout=3.0)
