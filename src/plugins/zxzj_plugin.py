...
class ZxzjPlugin(BasePlugin):
    name = "zxzj"
    display_name = "在线之家"
    version = "1.0.0"
    author = "pan-relay"
    description = "在线之家 数据源搜索插件"
    priority = 140
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://zxzj.org/search/%s.html" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://zxzj.org"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://zxzj.org", headers={"User-Agent": user_agents[0]}, timeout=3.0)
