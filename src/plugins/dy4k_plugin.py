...
class Dy4kPlugin(BasePlugin):
    name = "dy4k"
    display_name = "4K电影网"
    version = "1.0.0"
    author = "pan-relay"
    description = "4K电影网 数据源搜索插件"
    priority = 150
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://4kdy.vip/search/%s.html" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://4kdy.vip"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://4kdy.vip", headers={"User-Agent": user_agents[0]}, timeout=3.0)
