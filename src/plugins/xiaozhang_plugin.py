...
class XiaozhangPlugin(BasePlugin):
    name = "xiaozhang"
    display_name = "小张网盘"
    version = "1.0.0"
    author = "pan-relay"
    description = "小张网盘 数据源搜索插件"
    priority = 130
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://xiaozhang.com/api/search?kw=%s" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://xiaozhang.com"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://xiaozhang.com", headers={"User-Agent": user_agents[0]}, timeout=3.0)
