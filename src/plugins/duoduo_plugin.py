...
class DuoduoPlugin(BasePlugin):
    name = "duoduo"
    display_name = "多多资源"
    version = "1.0.0"
    author = "pan-relay"
    description = "多多资源 数据源搜索插件"
    priority = 140
    is_enabled = True
    timeout = 5.0
    ...
    def search(self, keyword: str) -> List[SearchResultItem]:
        ...
        if not keyword:
            return results
        url = "https://tv.yydsys.top/index.php/vod/search/wd/%s.html" % keyword
        headers = {"User-Agent": user_agents[0], "Referer": "https://tv.yydsys.top"}
        try:
    ...
    def health_check(self) -> Tuple[bool, str]:
        try:
            r = requests.get("https://tv.yydsys.top", headers={"User-Agent": user_agents[0]}, timeout=3.0)
