...
**内置SQLite数据库，支持本地资源、第三方API、Telegram公开频道与可扩展插件聚合搜索。支持完全解耦的开放REST API与小程序、APP快速接入。**
...
* **自动化转存换链**：已接入**夸克网盘、百度网盘、阿里云盘、UC 网盘、迅雷网盘、光鸭云盘、悟空网盘、移动云盘**8大主流网盘。
...
* **多渠道聚合搜索**：
  ...
  * **两步走REST API**：提供规范的 `/api/v1` REST接口（步骤 1 聚合查询 -> 步骤 2 自动转存替换），便于无缝对接微信小程序、Flutter/React Native APP、Telegram机器人或资源导航站。

## ✨ 项目特点

* **8大网盘能力全覆盖**：夸克、UC、百度、阿里云盘、迅雷、移动云盘、光鸭云盘、悟空网盘全量支持目录自动归档、上游垃圾广告文件智能过滤、以及专属引流文件自动植入。
...
| 网盘平台 | 自动转存换链 | 目录智能归档 | 上游广告净化 | 专属引流植入 | 过期自动清理 | 凭证类型 |
| :--- | :---: | :---: | :---: | :---: | :---: | :--- |
| **夸克网盘** | ✓ | ✓ | ✓ | ✓ | ✓ | Cookie |
| **UC 网盘** | ✓ | ✓ | ✓ | ✓ | ✓ | Cookie |
| **百度网盘** | ✓ | ✓ | ✓ | ✓ | ✓ | Cookie |
...
| 配置项 | 可选值 | 说明 |
| :--- | :--- | :--- |
| **前台搜索开关** | 开启/关闭 | 独立禁用前台Web搜索界面 (访客访问 `GET /` 自动重定向至后台管理登录页面 `/login`) |
| **API默认搜索范围** | `own`/`all` | API搜索默认检索范围 (`own`: 仅站长收益库 / `all`: 全网并发) |
| **转存API鉴权密钥** | 字符串/留空 | API转存鉴权密钥 (留空表示公开允许转存，设置后客户端请求需携带 `X-API-Key` 或 `Bearer Token`) |

*注：即使前台Web完全关闭，系统后台的开放API、管理后台及定时清理Worker（`storage_cleanup_service`）依然会独立正常运行。*
...
登录管理后台（`/admin`）进入 **系统配置 -> 云盘转存凭证** 即可配置8大网盘平台登录态：
...
* **中国移动云盘**：登录 `yun.139.com` 网页版，从请求标头中复制 `Authorization` 或 `jwtToken`。
