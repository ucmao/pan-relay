...
| **默认搜索范围** | `own`/`all` | 支持锁定 | 未传参 `scope` 时的检索范围 (`own`: 仅站长收益库 / `all`: 全网并发) |
| **默认单次结果上限** | 数字 (如 `50`) | 支持锁定 | 未传参 `limit` 时的单次最大条数上限 (防止消耗过量并发与带宽) |
| **默认清洗失效死链** | 开启/关闭 | 支持锁定 | 未传参 `filter_bad` 时默认自动剔除失效死链，保障 API 返回品质 |
| **默认实时健康探针** | 开启/关闭 | 支持锁定 | 未传参 `check_status` 时默认实时检测网盘存活并注入 `health_state` 状态 |
| **转存API鉴权密钥** | 字符串/留空 | - | 保护物理转存接口 `POST /api/v1/transfer` (设置后需携带 `X-API-Key` 或 Bearer Token) |

> 🔒 **防护锁 (Lock) 防穿透说明**：在管理后台勾选“锁定”后，外部请求即便传入参数（如 `?limit=1000` 或 `?check_status=true`）亦无法强行穿透覆盖站长预设，确保系统服务器资源安全。
