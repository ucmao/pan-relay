...
* **自动化转存换链**：已接入 **夸克网盘、百度网盘、阿里云盘、UC 网盘、迅雷网盘、光鸭云盘、悟空网盘、移动云盘** 8 大主流网盘。
  ...
  * **模式二：实时转存 (出链)**：前台或 API 搜索外部资源时按需实时转存并生成收益链接返回给用户，过期由系统定时任务自动清理释放网盘空间。
...
| 网盘平台 | 链接识别 | 批量转存 (入库) | 实时转存 (出链) | 凭证类型 |
| :--- | :------: | :-------------: | :-------------: | :------- |
| **夸克网盘** | ✓ | ✓ | ✓ | Cookie |
| **百度网盘** | ✓ | ✓ | ✓ | Cookie |
| **阿里云盘** | ✓ | ✓ | ✓ | Refresh Token |
...
| **光鸭云盘** | ✓ | ✓ | ✓ | Access Token / Refresh Token |
...
登录管理后台（`/admin`）进入 **系统配置 -> 云盘转存凭证** 即可配置 8 大网盘平台登录态：
